...
function ComparisonWorkspace({ sourceMode }) {
  ...
  useEffect(() => {
    return () => {
      if (leftObjectUrlRef.current) URL.revokeObjectURL(leftObjectUrlRef.current)
      if (rightObjectUrlRef.current) URL.revokeObjectURL(rightObjectUrlRef.current)
    }
  }, [])
  ...
  }, [bothLoaded, mismatchEnabled, mismatchMode, threshold, leftSrc, rightSrc, drawMismatch])
